...
> **Corrected while implementing 0.2.0.** Persistence is not gated on portal 1.21.0. The spec ties `restore_token` and `persist_mode` to **version 2 of the `RemoteDesktop` interface**, the same version that added `ConnectToEIS` ("This option was added in version 2 of this interface"). Confirmed by running it: xdg-desktop-portal **1.18.4** with xdg-desktop-portal-gnome 46.2 advertises `RemoteDesktop` v2, returned a `restore_token` from `Start`, and replaying it suppressed the dialog on the next run. Check the interface version, not the portal package version.
>
> Two further findings from that work, both things this document could not have known:
>
> - **The absolute-motion coordinate space arrives on the EI device, not from the screencast stream.** §5 is right that the stream is what makes absolute placement possible, but the geometry comes back as `ei_device.region` (offset, size, scale in logical pixels) — so a client needs the stream *granted*, never *connected*. No PipeWire dependency for placement. GNOME 46 reported `(0, 0, 1800, 1130, scale 1.0)`, matching the stream's logical size exactly.
> - **`reis`'s core is synchronous.** Its async modules (`tokio`, `async-io`, `calloop`) are optional integrations and none are default features; `ei::Context` is `new`/`read`/`pending_event`/`flush`, and `handshake::ei_handshake_blocking` is public. Paired with zbus's blocking API this means the whole Wayland path needs no async runtime, so `ashpd` — which is async-only — was not used.
